...
connect = sqlite3.connect("newOcel.sqlite")
c = connect.cursor()
c.execute("ATTACH DATABASE 'merged.sqlite' as 'merged'")

def create_new_event_OCEL(c):
...
def create_new_eventMapType_OCEL(c):
...
def create_new_object_object_OCEL(c):
...
def create_new_objectMapType_OCEL(c):
...
def create_new_eventObject_OCEL(c):
...
def create_new_eventOcelTypes_OCEL(c):
    ...
    for e in eventTypes:
        ...
        if len(allAttribute)==0:
            ...
            for a in allAttribute:
                ...
                c.execute(f"CREATE TABLE event_{a[0]}_{e[1].replace(" ","")}_temp AS SELECT eventID as ocel_id, eventTime as ocel_time, eventAttributeValue AS '{a[0]}' FROM merged.event NATURAL JOIN merged.eventAttributeValue Natural JOiN merged.eventAttribute WHERE eventAttributeName = '{a[0]}'  ")
...
def create_new_objectOcelTypes_OCEL(c):
    ...
    for e in objectTypes:
        ...
        if len(allAttribute)==0:
            ...
            for a in allAttribute:
                stra = stra + f"""'{a[0]}'"""+" TEXT, "
                list.append(f"object_{a[0]}_{e[1].replace(" ","")}_temp")
            
                c.execute(f"CREATE TABLE  object_{a[0]}_{e[1].replace(" ","")}_temp AS SELECT objectID as ocel_id, objectAttributeValTime as ocel_time, attributeValue AS '{a[0]}' FROM merged.objectAttributeValue Natural JOiN merged.objectAttribute WHERE objectAttributeName = '{a[0]}'  ")
...
create_new_eventMapType_OCEL(c)
create_new_object_object_OCEL(c)
create_new_objectMapType_OCEL(c)
create_new_event_OCEL(c)
create_new_object_OCEL(c)
create_new_eventObject_OCEL(c)
create_new_eventOcelTypes_OCEL(c)
create_new_objectOcelTypes_OCEL(c)
print(c.fetchall())
